    ...
    <div className="form-wrap">
      <div className="form-header">
        ...
        <div className="form-header-text">
          ...
          <p>Find brand deals that match your content</p>
        </div>
      </div>
      ...
      <div className="filter-bar">
        {['all', 'cpv', 'participation', 'one_time'].map((f) => (
          <button
            ...
            onClick={() => setFilter(f)}
          >
            {f === 'all' ? 'All Campaigns' : MODEL_LABELS[f]?.icon + ' ' + MODEL_LABELS[f]?.label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="campaigns-empty">
          ...
          <p className="empty-desc">Brands are joining VeroLinkr. Check back soon.</p>
        </div>
      ) : (
              ...
              <div className="campaign-card" key={campaign.id}>
                ...
                <div className="campaign-meta">
                  ...
                  {campaign.campaign_type === 'cpv' && (
                    <>
                      <div className="meta-item">
                        <span className="meta-label">Rate per View</span>
                        <span className="meta-value">₹{campaign.cpv_rate}</span>
                      </div>
                      <div className="meta-item">
                        <span className="meta-label">Milestone</span>
                        <span className="meta-value">Every {Number(campaign.milestone_views).toLocaleString()} views</span>
                      </div>
                      <div className="meta-item">
                  ...
                  {campaign.campaign_type === 'participation' && (
                    <>
                      <div className="meta-item">
                        <span className="meta-label">Your Payout</span>
                        <span className="meta-value" style={{ color: 'var(--vero-accent)', fontWeight: 600 }}>₹{Number(campaign.payout_per_post).toLocaleString()}</span>
                      </div>
                      <div className="meta-item">
                        <span className="meta-label">Slots Available</span>
                        <span className="meta-value">{campaign.max_creators} creators</span>
                      </div>
                    </>
                  )}
                  ...
                  {campaign.campaign_type === 'one_time' && (
                    <>
                      <div className="meta-item">
                        <span className="meta-label">Creators Needed</span>
                        <span className="meta-value">{campaign.max_creators}</span>
                      </div>
                      {campaign.content_only && (
                        <div className="meta-item">
                          <span className="meta-label">Type</span>
                          <span className="meta-value">Content for Ads (no public post)</span>
                        </div>
                      )}
                    </>
                  )}
                ...
                {campaign.campaign_type === 'cpv' ? (
                  <div className="cpv-join-info">
                    ⚡ CPV campaigns are open to all verified creators. Connect your social accounts to join automatically once verification launches.
                  </div>
                ) : !hasApplied ? (
